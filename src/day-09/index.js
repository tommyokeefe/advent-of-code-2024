...
const getCurrentEndFreeSpace = blocks => {
  const { count } = [...blocks].reverse().reduce(
    (acc, block) => {
      if (acc.foundUsed) {
        return acc
      }
...
const defragFiles = map => {
  let workingMap = map
  const startingIndex = [...workingMap]
    .reverse()
    .find(item => item.type === FILE).index

  for (let x = startingIndex; x > 0; x--) {
    const fileIndex = workingMap.findIndex(
      item => item.type === FILE && item.index === x
    )
    const file = workingMap[fileIndex]
    const spaceIndex = workingMap.findIndex(
      item => item.type === SPACE && item.count >= file.count
    )
    ...
    const FileAndLeftover =
      leftoverSpace > 0 ? [file, { type: SPACE, count: leftoverSpace }] : [file]
...
export function solution2(input) {
  ...
  return blocks.reduce(
    (total, block, index) => (block === '.' ? total : total + block * index),
    0
  )
}
